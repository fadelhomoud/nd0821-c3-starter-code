...
def test_model_train(data):
    ...
    ''' 
    Test that the model will have the same number of features and classes as the input data
    '''
...
def test_inference(data):
    ''' 
    Test that the inference function will return a numpy array with the same length as the input
    '''
    model = train_model(data.drop(columns=['y']), data.y)
